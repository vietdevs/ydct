...
using newPSG.PMS.Web.Auth;
using newPSG.PMS.Web.Controllers.Results;
using newPSG.PMS.Web.Models.Account;
using newPSG.PMS.Web.Models.Layout;
using newPSG.PMS.Web.MultiTenancy;
...
namespace newPSG.PMS.Web.Controllers
{
    public class AccountController : PMSControllerBase
    {
        ...
        public virtual async Task<JsonResult> Login(LoginViewModel loginModel, string returnUrl = "", string returnUrlHash = "")
        {
            ...
            string tenancyNameDoanhNghiep = ConfigurationManager.AppSettings["TENANTCY_NAME_DOANH_NGHIEP"];

            string tenancyNameCucHCC = ConfigurationManager.AppSettings["TENANTCY_NAME_CUC_HCC"];

            if (string.IsNullOrEmpty(loginModel.TenancyName))
            {
                if (loginModel.UsernameOrEmailAddress.Contains("/"))
                {
                    string[] arr = loginModel.UsernameOrEmailAddress.Split("/");
                    if (arr.Length > 1)
                    {
                        loginModel.TenancyName = arr[0];
                        loginModel.UsernameOrEmailAddress = arr[1];
                        if (loginModel.TenancyName.ToLower() == "host")
                        {
                            loginModel.TenancyName = null;
                        }
                    }
                }
                else if (loginModel.UsernameOrEmailAddress.ToLower().Contains("@hcc.gov.vn"))
                {
                    var tk = loginModel.UsernameOrEmailAddress.Trim().ToLower();
                    tk = tk.Replace("@hcc.gov.vn", "");
                    if (!string.IsNullOrEmpty(tk))
                    {
                        if (tk.Contains("."))
                        {
                            string[] arr = tk.Split(".");
                            loginModel.TenancyName = arr.Last();
                        }
                        else
                        {
                            if (!string.IsNullOrEmpty(tenancyNameCucHCC))
                            {
                                loginModel.TenancyName = tenancyNameCucHCC;
                            }
                            else
                            {
                                loginModel.TenancyName = "hcc";
                            }
                        }
                    }
                }
                ...
                {
                    if (!string.IsNullOrEmpty(tenancyNameDoanhNghiep))
                    {
                        loginModel.TenancyName = tenancyNameDoanhNghiep;
                    }
                    else
                    {
            ...
            using (UnitOfWorkManager.Current.SetTenantId(tenantId))
            {
                ...
                {
                    await SetCustomSession(loginResult.User, loginResult.Identity);
                }
        ...
        private async Task SignInAsync(User user, ClaimsIdentity identity = null, bool rememberMe = false)
        {
            if (identity == null)
            {
                ...
                //SetCustomSession
                await SetCustomSession(user, identity);
            }
        ...
        public ActionResult Register()
        {
            ...
            {
                TenancyName = _tenancyNameFinder.GetCurrentTenancyNameOrNull(),
                PasswordComplexitySetting = JsonConvert.DeserializeObject<PasswordComplexitySetting>(SettingManager.GetSettingValue(AppSettings.Security.PWComplexity))
            });
        }
        ...
        public virtual async Task<ActionResult> ResetPassword(ResetPasswordViewModel model)
        {
            ...
            var setting = await SettingManager.GetSettingValueForUserAsync(AppSettings.Security.PWComplexity, tenantId, userId);
        ...
        public PartialViewResult AppLogo(string appPath, int width = 168, int height = 33)
        {
            ...
            if (string.IsNullOrEmpty(tenancyName))
            {
                ViewBag.TenantCustomLogoUrl = appPath + "Common/Images/login-logo.png";
            }
            else
            {
                using (_unitOfWorkManager.Begin())
                {
                    using (_unitOfWorkManager.Current.SetTenantId(null))
                    {
                        ...
                        ViewBag.TenantCustomLogoUrl = tenant != null && tenant.LogoId.HasValue
                            ...
                            : appPath + "Common/Images/login-logo.png";
                    }
                }
            }
        ...
        private async Task SetCustomSession(User User, ClaimsIdentity identity = null)
        {
            var _userSession = User.MapTo<UserLoginInfoDto>();
            _userSession.ThuTucEnum = new List<int>();

            // get thu tu
            var currentGrantedPermissions = await _userManager.GetGrantedPermissionsAsync(User);
            for (int i = 1; i <= 90; i++)
            {
                string tt = (i < 10) ? $"0{i}" : i.ToString();
                string permissinName = $"Pages.ThuTuc{tt}";
                if (currentGrantedPermissions.Any(x => x.Name == permissinName))
                {
                    _userSession.ThuTucEnum.Add(i);
                }
            }
            string _jUser = JsonConvert.SerializeObject(_userSession);
            identity.AddClaim(new Claim(AppConsts.KeyClaimSession, _jUser));
        }

        #endregion custom Session
    }
}