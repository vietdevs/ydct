(function () {
    appModule.directive('app.directives.dirmotcuaphancong', ['$compile', '$timeout', '$templateRequest', '$filter', 'appSession', '$linq',
        function ($compile, $timeout, $templateRequest, $filter, appSession, $linq) {
            var controller = ['$scope', '$timeout', '$window', '$filter', '$uibModal', '$state',
                function ($scope, $timeout, $window, $filter, $uibModal, $state) {
                    ...
                    var mainFunc = () => {
                        vm.go_motcuaphancongchung = () => {
                            $state.go('motcuaphancongchung');
                        };
                    }
                    var loadTotalLabel = () => {
                        let _req = {
                        ...
                        abp.services.app.thuTucReport.getTotalFormCase(_req)
            ...
            return {
                ...
                link: function (scope, elem, attr, ctrl) {
                    $templateRequest("/App/_directives/dirVanThuDashBoard/dirMotCuaPhanCong.html").then(function (html) {
