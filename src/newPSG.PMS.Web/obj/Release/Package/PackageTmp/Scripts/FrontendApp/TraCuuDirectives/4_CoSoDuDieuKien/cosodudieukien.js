(function () {
        ...
        var controller = ['$scope', '$http', '$window',
            function ($scope, $http, $window) {
                ...
                vm.filter = {
                    ...
                    tinhId: null,
                    soCongBo: null,
                    loaiSanPhamDangKy: null,
                    sanPhamDuocCapPhep: null
                ...
                vm.gridHoSoDataSource = new kendo.data.DataSource({
                    transport: {
                        read: function (options) {
                            ...
                            vm.loading = true;
                            $http.post('/api/services/app/traCuuHoSo04/getListHoSoTraCuuPaging', $.extend(vm.filter, vm.requestParams))
                                .then(function (result) {
                                    ...
                                    if (result.data.result.items) {
                                        result.data.result.items.forEach(function (item, idx) {
                                            item.STT = idx + 1;
                                            item.loaiSanPhamDangKys = JSON.parse(item.loaiSanPhamDangKy);
                                        });
                ...
                vm.columnGridHoSo = [
                    ...
                    {
                        field: "loaiSanPhamDangKys",
                        title: "Nhóm sản phẩm",
                        width: "350px",
                        template: `<div ng-repeat="item in this.dataItem.loaiSanPhamDangKys">- {{item.tenLoaiHinh}} {{item.dangSanPham}}</div>`
                    },
                    {
                        field: "ngayTraKetQua",
                        headerAttributes: { style: "text-align: center;" },
    ...
    MyApp.directive('tracuu.dongnai.cosodudieukien', ['$compile', '$timeout', '$templateRequest',
        function ($compile, $timeout, $templateRequest) {
            return {
                ...
                link: function (scope, elem, attr, ctrl) {
                    $templateRequest("/Scripts/FrontendApp/TraCuuDirectives/4_CoSoDuDieuKien/Index.html").then(function (html) {
    ...
    MyApp.directive('tracuu.danang.cosodudieukien', ['$compile', '$timeout', '$templateRequest',
        function ($compile, $timeout, $templateRequest) {
            return {
                restrict: 'EA',
                replace: true,
    ...
    MyApp.directive('tracuu.hanoi.cosodudieukien', ['$compile', '$timeout', '$templateRequest',
        function ($compile, $timeout, $templateRequest) {
            return {
                restrict: 'EA',
                replace: true,
    ...
    MyApp.directive('tracuu.laocai.cosodudieukien', ['$compile', '$timeout', '$templateRequest',
        function ($compile, $timeout, $templateRequest) {
            return {
                restrict: 'EA',
                replace: true,
    ...
    MyApp.directive('tracuu.vinhphuc.cosodudieukien', ['$compile', '$timeout', '$templateRequest',
        function ($compile, $timeout, $templateRequest) {
            return {
                restrict: 'EA',
                replace: true,
