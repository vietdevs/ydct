(function () {
    appModule.controller('danhmuc.views.thutuc.createOrEditModal', [
        '$scope', '$uibModalInstance', 'abp.services.app.thuTuc', 'loaihoso', 'FileUploader', 'baseService', 'appSession', 'abp.services.app.commonLookup',
        function ($scope, $uibModalInstance, thuTucService, loaihoso, fileUploader, baseService, appSession, commonLookupService) {
            //variable
            console.log('danhmuc.views.thutuc.createOrEditModal')
            var vm = this;
            vm.saving = false;
            vm.loaihoso = {
                isActive: true,
            ...
            vm.uploader = new fileUploader({
                url: abp.appPath + 'File/UploadIconNhomSanPham',
                headers: {
            ...
            vm.uploader.onSuccessItem = function (fileItem, response, status, headers) {
                if (response.success) {
                    if (response.result.data) {
                        ...
                        vm.loaihoso.dataImage = vm.uploadedFileName;
                    }
                } else {
            ...
            vm.save = function () {
                vm.saving = true;
                if (vm.validForm() == false) {
                    vm.saving = false; return;
                }

                thuTucService.createOrUpdate(vm.loaihoso).then(function (result) {
                    if (result.data) {
