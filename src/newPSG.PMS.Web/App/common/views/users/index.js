(function () {
    appModule.controller('common.views.users.index', [
        '$scope', '$uibModal', '$stateParams', 'uiGridConstants', 'abp.services.app.user', 'appSession',
        function ($scope, $uibModal, $stateParams, uiGridConstants, userService, appSession) {
            var vm = this;
            ...
            vm.userColumns = [
                {
                    ...
                    template: '<div class=\"ui-grid-cell-contents\">' +
                        ...
                        '    <ul uib-dropdown-menu>' +
                        '      <li><a ng-if="(vm.user.userName==\'adminx\' || this.dataItem.userName!=\'adminx\') && vm.tenant == null && vm.permissions.impersonation && this.dataItem.id != vm.currentUserId" ng-click="vm.impersonate(this.dataItem)">' + app.localize('LoginAsThisUser') + '</a></li>' +
                        '      <li><a ng-if="vm.permissions.edit" ng-click="vm.editUser(this.dataItem)">' + app.localize('Edit') + '</a></li>' +
                        '      <li><a ng-if="vm.permissions.changePermissions" ng-click="vm.editPermissions(this.dataItem)">' + app.localize('Permissions') + '</a></li>' +
                        '      <li><a ng-click="vm.unlockUser(this.dataItem)">' + app.localize('Unlock') + '</a></li>' +
                        '      <li><a ng-if="vm.permissions.delete" ng-click="vm.deleteUser(this.dataItem)">' + app.localize('Delete') + '</a></li>' +
                        '    </ul>' +
                        '  </div>' +
                        '</div>',
                },
                ...
                },
                {
                    ...
                    template:
                        '<div class=\"ui-grid-cell-contents\">' +
                        '  <span ng-show="this.dataItem.isEmailConfirmed" class="label label-success">' + app.localize('Yes') + '</span>' +
                        '  <span ng-show="!this.dataItem.isEmailConfirmed" class="label label-default">' + app.localize('No') + '</span>' +
                        '</div>',
                    minWidth: 120
                },
                {
                    ...
                    read: function (options) {
                        ...
                        userService.getUsers($.extend({ filter: vm.filterText }, vm.requestParams))
                            .then(function (result) {
                                ...
                                result.data.items = addRoleNamesField(result.data.items);
            ...
            vm.deleteUser = function (user) {
                if (user.userName == app.consts.userManagement.defaultAdminUserName) {
                    abp.message.error(app.localize("{0}UserCannotBeDeleted", app.consts.userManagement.defaultAdminUserName));
                    return;
                }
