(function () {
    appModule.controller('host.views.tenants.index', [
        '$scope', '$stateParams', '$uibModal', 'uiGridConstants', 'abp.services.app.tenant', 'abp.services.app.commonLookup', 'lookupModal', 'appSession',
        function ($scope, $stateParams, $uibModal, uiGridConstants, tenantService, commonLookupService, lookupModal, appSession) {
            var vm = this;

            vm.getTinhName = function (tinhId) {
                var arrTinh = appSession.get_tinh();
                if (tinhId) {
                    var tinhObj = arrTinh.find(function (element) {
                        return element.id == tinhId;
                    });
            ...
            vm.tenantGridOptions = {
                ...
                columnDefs: [
                    ...
                    },
                    {
                        ...
                        field: 'tenancyName',
                        cellTemplate:
                            '<div class=\"ui-grid-cell-contents\" title="Id = {{row.entity.id}}">' +
                ...
                tenantService.getTenants({
                    ...
                    if (result.data.items) {
                        result.data.items.forEach(function (item) {
                            item.tinhName = vm.getTinhName(item.tinhId);
                        });
                    }
