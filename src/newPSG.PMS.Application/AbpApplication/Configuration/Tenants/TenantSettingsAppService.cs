...
namespace newPSG.PMS.Configuration.Tenants
{
    ...
    public class TenantSettingsAppService : PMSAppServiceBase, ITenantSettingsAppService
    {
        private readonly IMultiTenancyConfig _multiTenancyConfig;
        private readonly IAbpZeroLdapModuleConfig _ldapModuleConfig;
        private readonly ITimeZoneService _timeZoneService;
        private readonly IBinaryObjectManager _binaryObjectManager;
        private readonly ITenantSettingsDomainService _tenantSettingsDomain;
        ...
        public TenantSettingsAppService(
            ...
            ITenantSettingsDomainService tenantSettingsDomain)
        {
            _multiTenancyConfig = multiTenancyConfig;
            _ldapModuleConfig = ldapModuleConfig;
            _timeZoneService = timeZoneService;
            _binaryObjectManager = binaryObjectManager;
            _tenantSettingsDomain = tenantSettingsDomain;
        }
        ...
        public async Task<TenantSettingsEditDto> GetAllSettings()
        {
            try
            {
                var settings = new TenantSettingsEditDto
                {
                    ...
                    LienThong = await GetLienThongInfoSettingsAsync()
                };
        ...
        {
            var passwordComplexitySetting = await SettingManager.GetSettingValueAsync(AppSettings.Security.PWComplexity);
            var defaultPasswordComplexitySetting = await SettingManager.GetSettingValueForApplicationAsync(AppSettings.Security.PWComplexity);
        ...
        //Payment
        private async Task<PaymentSettingDto> GetPaymentSettingsAsync()
        {
            var _keypay_merchant_key = await SettingManager.GetSettingValueAsync(AppSettings.Payment.KEYPAY_MERCHANT_KEY);
            var _keypay_merchant_code = await SettingManager.GetSettingValueAsync(AppSettings.Payment.KEYPAY_MERCHANT_CODE);
            var settings = new PaymentSettingDto
            {
                KeypayMerchantKey = _keypay_merchant_key,
                KeypayMerchantCode = _keypay_merchant_code
            };
        ...
        //LienThong
        private async Task<LienThongInfoSettingDto> GetLienThongInfoSettingsAsync()
        {
            try
            {
                return await _tenantSettingsDomain.GetLienThongInfoSettingsAsync(AbpSession.GetTenantId());
            }
            catch (Exception ex)
            {
                Logger.Error(ex.Message);
        ...
        private async Task UpdateSecuritySettingsAsync(SecuritySettingsEditDto settings)
        {
            if (settings.UseDefaultPasswordComplexitySettings)
            {
                await SettingManager.ChangeSettingForTenantAsync(
                    AbpSession.GetTenantId(),
                    AppSettings.Security.PWComplexity,
        ...
        //Payment
        private async Task UpdatePaymentSettingsAsync(PaymentSettingDto input)
        {
            await SettingManager.ChangeSettingForTenantAsync(AbpSession.GetTenantId(), AppSettings.Payment.KEYPAY_MERCHANT_KEY, input.KeypayMerchantKey);
            await SettingManager.ChangeSettingForTenantAsync(AbpSession.GetTenantId(), AppSettings.Payment.KEYPAY_MERCHANT_CODE, input.KeypayMerchantCode);
        }
