...
namespace newPSG.PMS.Authorization.Users.Profile
{
    ...
    public class ProfileAppService : PMSAppServiceBase, IProfileAppService
    {
        ...
        public async Task<string> UpdateCurrentUserProfile(CurrentUserProfileEditDto input)
        {
            try
            {
                using (CurrentUnitOfWork.DisableFilter(AbpDataFilters.MayHaveTenant))
                {
                    if (!string.IsNullOrEmpty(input.EmailAddress) && UserManager.Users.Any(x => x.EmailAddress == input.EmailAddress))
                    {
                        return "email_da_co";
                    }
        ...
        {
            var settingValue = await SettingManager.GetSettingValueAsync(AppSettings.Security.PWComplexity);
        ...
        {
            var passwordComplexitySettingValue = await SettingManager.GetSettingValueAsync(AppSettings.Security.PWComplexity);
