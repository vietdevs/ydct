...
{
    public interface IXuLyHoSoView98AppService : IApplicationService
    {
        Task<dynamic> GetViewHoSo(long hoSoId);
        Task<dynamic> GetHistory(long hoSoId);
    }
    [AbpAuthorize]
    public class XuLyHoSoView98AppService : PMSAppServiceBase, IXuLyHoSoView98AppService
    {
        ...
        public XuLyHoSoView98AppService(IRepository<XHoSo, long> hoSoRepos,
            ...
            try
            {
                if (hoSoId > 0)
                {
                    ...
                    var listCongVan = await (from hsxl in _hoSoXuLyRepos.GetAll()
                                             where hsxl.HoSoId == hoSoId && hsxl.HoSoIsDat == false
                    ...
                    var _urlBanDangKy = "/Report98/TemplateHoSo?hoSoId=" + hoSo.Id;
                    if (hoSo.IsCA == true && !String.IsNullOrEmpty(hoSo.DuongDanTepCA))
                    {
                        _urlBanDangKy = string.Format("/File/GoToViewTaiLieu?filename={0}.pdf&url={1}",hoSo.MaHoSo.Replace("/","_"), hoSo.DuongDanTepCA);
                    }
                        ...
                        foreach (var item in _listYKien)
                        {
                            ...
                            switch (item.DonViXuLy)
                            {
                                ...
                                case (int)CommonENum.DON_VI_XU_LY.VAN_THU:
                                    ...
                                    if(item.ActionEnum == (int)CommonENum.FORM_FUNCTION.VAN_THU_DUYET)
                                    {
                                        item.HanhDongXuLy = "Duyệt hồ sơ";
                                    }
                                    break;
