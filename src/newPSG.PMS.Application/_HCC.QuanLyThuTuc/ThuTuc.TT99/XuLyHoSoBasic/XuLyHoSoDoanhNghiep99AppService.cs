...
namespace newPSG.PMS.Services
{
    ...
    public interface IXuLyHoSoDoanhNghiep99AppService : IApplicationService
    {
        ...
        Task<long> CreateOrUpdateHoSo(CreateOrUpdateHoSo99Input input);
        ...
        Task<int> UpdateKySoHoSo(UpdateKySo99InputDto input);

        #endregion Ký số hồ sơ

        #region Thanh toán Keypay

        Task<object> ChuyenThanhToanKeyPay(KyVaThanhToan99InputDto input);
        ...
        Task<long> ThanhToanChuyenKhoan(ThanhToanChuyenKhoan99InputDto input);
        ...
        List<HoSoTepDinhKem99Dto> GetHoSoTepDinhKem(long hoSoId);

        bool CopyFileTaiLieuDaTaiVaoHoSo(CopyFileTaiLieuDaTaiVaoHoSo99Input input);
    ...
    [AbpAuthorize]
    public class XuLyHoSoDoanhNghiep99AppService : PMSAppServiceBase, IXuLyHoSoDoanhNghiep99AppService
    {
        private readonly int _thuTucId = (int)CommonENum.THU_TUC_ID.THU_TUC_99;
        ...
        public XuLyHoSoDoanhNghiep99AppService(
        ...
        public async Task<long> CreateOrUpdateHoSo(CreateOrUpdateHoSo99Input input)
        {
            long _id = 0;
            if (input.HoSo.Id > 0)
            {
            ...
            {
                var lstUpload = (from uploadObj in _uploadFileRepos.GetAll().Where(x => _arrUploadFileId.Contains(x.Id)) select uploadObj).ToList();
                foreach(var upObj in lstUpload)
                {
                    upObj.DaSuDung = true;
                    _uploadFileRepos.Update(upObj);
                }
            }
        ...
        private async Task<long> UpdateHoSo(CreateOrUpdateHoSo99Input input)
        {
            try
            {
                ...
                if (updateData != null)
                {
                    ...
                    foreach (var tailieu in input.Teps)
                    {
                        ...
                        tailieu.HoSoId = updateData.Id;
                        if (!String.IsNullOrEmpty(tailieu.TenTep))
                        {
                            insertTailieu.Id = 0;
                            insertTailieu.DaTaiLen = true;
                            insertTailieu.IsActive = true;
                            insertTailieu.DuongDanTep = tailieu.TenTep;
                        }
                    ...
                    {
                        updateData.SoDangKy = _capSoThuTucAppService.SinhSoDangKy(updateData.DoanhNghiepId.Value, (int)CommonENum.THU_TUC_ID.THU_TUC_99);
                    }
        ...
        private async Task<long> CreateHoSo(CreateOrUpdateHoSo99Input input)
        {
            try
            {
                ...
                insertInput.SoDangKy = _capSoThuTucAppService.SinhSoDangKy(input.HoSo.DoanhNghiepId.Value, (int)CommonENum.THU_TUC_ID.THU_TUC_99);
        ...
        public async Task<int> UpdateKySoHoSo(UpdateKySo99InputDto input)
        {
            try
            {
                if (input.HoSoId > 0)
        ...
        public async Task<object> ChuyenThanhToanKeyPay(KyVaThanhToan99InputDto input)
        {
            try
            {
                var hoSo = await _hoSoRepos.GetAsync(input.HoSoId);
                ...
                if (hoSo.TrangThaiHoSo.Value == (int)CommonENum.TRANG_THAI_HO_SO.CHO_DOANH_NGHIEP_THANH_TOAN)
                {
                    ...
                    thanhtoan.PhanHeId = (int)CommonENum.THU_TUC_ID.THU_TUC_99;
                    ...
                    thanhtoan.MaDonHang = CommonENum.GetEnumDescription(THU_TUC_ID.THU_TUC_99) + ".KP." + hoSo.Id;
                ...
                if (hoSoId > 0)
                {
                    ...
                    if (hoSo != null)
                    {
                        ...
                        var listThanhToan = from t in _thanhToanRepos.GetAll()
                                            ...
                                            && t.PhanHeId == (int)CommonENum.THU_TUC_ID.THU_TUC_99
                                            && t.HoSoId == hoSo.Id
                                            select t;
                        //Cập nhật thanh toán
        ...
        public async Task<long> ThanhToanChuyenKhoan(ThanhToanChuyenKhoan99InputDto input)
        {
            try
            {
                long idThanhToan = 0;
        ...
        public async Task NopHoSoDeRaSoat(long HoSoId)
        {
            try
            {
                using (var unitOfWork = _unitOfWorkManager.Begin())
                {
                    ...
                    //Update trang thai cua ho so doanh nghiep
        ...
        public async Task NopHoSoMoi(XHoSo hoSo, bool isAdmin = false, bool isKeToan = false)
        {
            using (var unitOfWork = _unitOfWorkManager.Begin())
            {
                ...
                if (phongBanIdXuLy > 0)
                {
                    hoSoXuLy.DonViXuLy = (int)CommonENum.DON_VI_XU_LY.PHONG_BAN_PHAN_CONG;
                }
                else
                {
                    hoSoXuLy.DonViXuLy = (int)CommonENum.DON_VI_XU_LY.MOT_CUA_PHAN_CONG;
                }
            ...
            try
            {
                ...
                if (hoSo.TrangThaiHoSo == (int)CommonENum.TRANG_THAI_HO_SO.SUA_DOI_BO_SUNG)
                {
                    ...
                    if (hoSoXuLyOld != null)
                    {
                        ...
                        hoSoXuLy.ChuyenVienThuLyId = hoSoXuLyOld.ChuyenVienThuLyId;
                        hoSoXuLy.ChuyenVienPhoiHopId = hoSoXuLyOld.ChuyenVienPhoiHopId;
                        hoSoXuLy.PhoPhongId = hoSoXuLyOld.PhoPhongId;
                        hoSoXuLy.TruongPhongId = hoSoXuLyOld.TruongPhongId;
                        hoSoXuLy.LanhDaoCucId = hoSoXuLyOld.LanhDaoCucId;
                        hoSoXuLy.LanhDaoBoId = hoSoXuLyOld.LanhDaoBoId;
                        hoSoXuLy.VanThuId = hoSoXuLyOld.VanThuId;

                        hoSoXuLy.DonViGui = (int)CommonENum.DON_VI_XU_LY.DOANH_NGHIEP;
                        hoSoXuLy.DonViXuLy = (int)CommonENum.DON_VI_XU_LY.CHUYEN_VIEN_THAM_XET;
                        hoSoXuLy.NguoiXuLyId = hoSoXuLyOld.ChuyenVienThuLyId;
        ...
        public bool CopyFileTaiLieuDaTaiVaoHoSo(CopyFileTaiLieuDaTaiVaoHoSo99Input input)
        {
            bool ret = false;
            try
            {
                ...
                if (Directory.Exists(tempPath))
                {
                    foreach (var fileDaTai in input.DanhSachFileDaTai)
                    {
                        ...
                        {
                            var insertHoSoTep = new HoSoTepDinhKem99()
                            {
                                HoSoId = input.HoSoId,
        ...
        private string GetPathSaveHoSoFolder(long hoSoId = 0)
        {
            ...
            if (hoSoId == 0)
            {
                return $@"HoSo-TT99\\{strTinh}\\{maSoThue}\\{ DateTime.Now.Year.ToString()}";
            }
            return $@"HoSo-TT99\\{strTinh}\\{maSoThue}\\{ DateTime.Now.Year.ToString()}\\{hoSoIdFolder}";
        }

        public List<HoSoTepDinhKem99Dto> GetHoSoTepDinhKem(long hoSoId)
        {
            var query = from tep in _hoSoTepDinhKemRepos.GetAll()
                        where tep.HoSoId == hoSoId
                        where tep.IsActive == true
                        select new HoSoTepDinhKem99Dto
                        {
                            HoSoId = tep.HoSoId,
