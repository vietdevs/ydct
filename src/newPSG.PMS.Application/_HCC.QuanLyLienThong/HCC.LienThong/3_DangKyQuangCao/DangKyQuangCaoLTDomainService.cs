...
{
    public interface IDangKyQuangCaoLTDomainService : IDomainService
    {
    ...
    public class DangKyQuangCaoLTDomainService : PMSDomainServiceBase, IDangKyQuangCaoLTDomainService
    {
        ...
        public async Task<PagedResultDto<XHoSoLienThong>> GetListHoSoPaging(XPagingInput input, int? chiCucId)
        {
            try
            {
                ...
                if (dataGrids != null && dataGrids.Count > 0)
                {
                    ...
                    {
                        //var sanPhams = JsonConvert.DeserializeObject<List<SanPhamQuangCaoDto>>(item.JsonSanPham);
                        //item.TenSanPham = string.Join(", ", sanPhams.Select(a => a.TenThucPham));
        ...
        private IQueryable<XHoSoLienThong> QueryAll(XPagingInput input, int chiCucId)
        {
            ...
            var query = (from hoso in _vfaHoSoRepos.GetAll()
                         ...
                         select new XHoSoLienThong
                         {
                             ...
                             LoaiQuangCaoJson = hoso.LoaiQuangCaoJson,
                             JsonSanPham = hoso.JsonSanPham,
                             GiayTiepNhan = hoso.GiayTiepNhan,
                             GiayTiepNhanFull = hoso.GiayTiepNhanFull,
                             Guid = hoso.Guid,
                             LoaiQuangCaoKhacJson = hoso.LoaiQuangCaoKhacJson,
                             //DoanhNghiep
                             DiaChi = dn.DiaChi,
